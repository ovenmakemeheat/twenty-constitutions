...
import { loadData, getOverviewStats, getEraStats } from "~/data/constitutions.server"
import { ERA_LABELS, ERAS_ORDERED, THEMES_ORDERED, THEME_SHORT, type ConstitutionDoc } from "~/data/types"
...
import { RegimeBar, DocTypeDonut, ThemeHeatmap, EraTable } from "~/components/charts"
...
export async function loader(_: Route.LoaderArgs) {
  ...
  return { docs, stats, timelineDocs, eraThemeMatrix, maxThemeScore, eraBreakdown }
}
...
export default function Home({ loaderData }: Route.ComponentProps) {
  const { docs, stats, timelineDocs, eraThemeMatrix, maxThemeScore, eraBreakdown } = loaderData
  ...
  const filteredDocs = useMemo(() => {
    ...
    if (regimeFilter.size > 0) result = result.filter((d) => regimeFilter.has(d.regime_type))
    if (typeFilter.size > 0) result = result.filter((d) => typeFilter.has(d.doc_type))
  ...
  const activeEraLabel = eraFilter ? ERA_LABELS[eraFilter as keyof typeof ERA_LABELS] : null
  const hasFilters = !!(query || regimeFilter.size > 0 || typeFilter.size > 0 || eraFilter)
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-screen-xl items-center justify-between px-6 py-3">
          <div>
            ...
            <span className="ml-3 text-xs text-muted-foreground">รัฐธรรมนูญแห่งราชอาณาจักรไทย</span>
          </div>
          <nav className="flex items-center gap-4 text-xs font-medium uppercase tracking-wider text-muted-foreground">
      ...
      <section className="relative overflow-hidden border-b border-border bg-card">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 py-14">
          ...
          >
            TWENTY<br />CONSTITUTIONS<br />
            <span style={{ color: "var(--gold)" }}>DIGITIZATION.</span>
          </h1>
          <p className="mt-4 max-w-lg text-sm text-muted-foreground">
            89 years of Thai constitutional history — 38 documents, 2,797 sections, 13 thematic dimensions.
          </p>
          <div className="mt-6 flex flex-wrap items-center gap-2">
            ...
            >
              <svg width="14" height="14" viewBox="0 0 32 32" fill="none" aria-hidden="true">
                ...
                <path d="M8 24V8h4v8l6-8h5l-7 9 7 7h-5l-6-7v7H8z" fill="currentColor" />
              </svg>
              Kaggle
            </a>
            <a
      ...
      <div className="mx-auto max-w-screen-xl space-y-10 px-6 py-8">
        ...
        <section>
          ...
          <div className="mt-3 grid grid-cols-2 gap-px border border-border bg-border sm:grid-cols-3 lg:grid-cols-6">
            ...
            <StatCard value={stats.totalSections.toLocaleString()} label="Sections" />
            <StatCard value={`${stats.yearMin}–${stats.yearMax}`} label="Year range" sub={`${stats.yearMax - stats.yearMin} years`} />
            <StatCard value={stats.totalFullDocs} label="Full constitutions" sub={`${stats.totalDocs - stats.totalFullDocs} amendments & interim`} />
            <StatCard value={stats.longestDoc.name_short} label="Longest document" sub={`${stats.longestDoc.section_count} sections`} />
            <StatCard value={(stats.totalWords / 1000).toFixed(0) + "k"} label="Total words" />
          </div>
        </section>
            ...
            <div className="mt-5 flex gap-5 border-t border-border pt-3">
              {(["civilian", "semi_military", "military"] as const).map((r) => (
                <div key={r} className="flex items-center gap-1.5">
                  <div className={cn("h-2.5 w-5 shrink-0", REGIME_BAR_CLASSES[r])} />
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{REGIME_LABEL[r]}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Darker = stronger thematic presence. Click a row to filter the constitution table below.
          </p>
          <div className="mt-3 border border-border bg-card p-5">
            <ThemeHeatmap
        ...
        <section>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <div className="relative">
              <svg className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" width="12" height="12" viewBox="0 0 12 12" fill="none">
              ...
              <input
                ...
                className="h-7 rounded-sm border border-border bg-card pl-7 pr-3 text-xs text-foreground placeholder:text-muted-foreground focus:border-ink-60 focus:outline-none"
                style={{ width: 220 }}
              />
              {query && (
                <button
            ...
            {(["civilian", "semi_military", "military"] as const).map((r) => (
              <button
                ...
                className={cn(
                  "rounded-sm border px-2 py-1 text-[10px] font-medium uppercase tracking-wider transition-colors",
                  regimeFilter.has(r)
                    ? REGIME_CLASSES[r]
            ...
            {(["full", "amendment", "interim"] as const).map((t) => (
              <button
                ...
                className={cn(
                  "rounded-sm px-2 py-1 text-[10px] font-medium uppercase tracking-wider transition-colors",
                  typeFilter.has(t)
            ...
            {hasFilters && (
              <button
                onClick={() => { setQuery(""); setRegimeFilter(new Set()); setTypeFilter(new Set()); setEraFilter(null) }}
                ...
                <tr className="border-b border-border">
                  {[
                    ...
                    <th
                      key={label}
                      className={cn(
                        "px-4 py-2.5 text-left font-medium uppercase tracking-wider text-muted-foreground",
                ...
                {filteredDocs.length === 0 ? (
                  ...
                  filteredDocs.map((doc) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-2 tabular-nums text-muted-foreground">{doc.year_ce}</td>
                        <td className="px-4 py-2 font-medium text-foreground">{doc.name_short}</td>
                        <td className="px-4 py-2"><DocTypeBadge type={doc.doc_type} /></td>
                        <td className="px-4 py-2 text-muted-foreground">{ERA_LABELS[doc.era]}</td>
                        <td className="px-4 py-2"><RegimeBadge regime={doc.regime_type} /></td>
                        <td className="px-4 py-2 tabular-nums text-foreground">{doc.section_count}</td>
                        <td className="px-4 py-2 text-muted-foreground">
                          {topTheme ? (THEME_SHORT[topTheme[0]] ?? topTheme[0]) : "—"}
                        </td>
                      </tr>
                    )
                  })
      ...
      <footer className="mt-12 border-t border-border bg-card">
        <div className="mx-auto max-w-screen-xl px-6 py-4 text-xs text-muted-foreground">
          <span>Made with Claude </span>
          <span style={{ color: "var(--gold)" }}>♥</span>
          <span> by ทีมงาน Production ชุมชนนิมนต์ยิ้ม season 67 และ สภาผู้แทนราษฎรอีสานใต้ชุดที่ 67</span>
        </div>
      </footer>

      {/* ── Doc Drawer ── */}
      {activeDoc && (
        <DocDrawer doc={activeDoc} allDocs={docs} onClose={() => setActiveDoc(null)} />
      )}
    </div>
  )
}
